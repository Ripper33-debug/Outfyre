    ...
    <section id="results" className="section-padding bg-void">
      <ScrollReveal className="mb-16">
        ...
        <h2 className="font-display text-4xl md:text-5xl lg:text-6xl font-extrabold text-white max-w-3xl">
          Proof we ship — and keep shipping
        </h2>
      </ScrollReveal>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {METRICS.map((metric, i) => (
          <div
            key={metric.label}
            className="group relative p-8 rounded-2xl bg-surface border border-white/[0.06] hover:border-ember/30 transition-all duration-500 hover:shadow-ember"
          >
            ...
            <div className="absolute inset-0 rounded-2xl bg-gradient-fire opacity-0 group-hover:opacity-[0.03] transition-opacity duration-500 pointer-events-none" />
          </div>
        ))}
      </div>
    </section>
